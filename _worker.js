...
async function chekURL(url) {
	try {
	  ...
	  if (r.ok) {return url.replace(/^https?:\/\//, '');} else {return null;}
	} catch (e) {
...
function 检查编码(rawUrl) {
  ...
  try {
  // 尝试 Base64 解码
  const decodedStr = atob(inputStr);
  // 如果解码后是 URL，则返回解码后的 URL
  if (decodedStr.startsWith('http://') || decodedStr.startsWith('https://')) {
  	return decodedStr;
  }
  } catch (e) {
  // 如果解码失败（比如输入不是 Base64），忽略错误
...
async function 变量数组整理(configString) {
  // 将文本分割成行，并过滤掉空行
  ...
  try {
    ...
    console.error("解析错误，请检查文本格式是否正确。", e);
    return null;
  }
}
...
export default {
	async fetch(request, env) {
		if (env.RawURL) RawURL = await 检查编码(env.RawURL);
		// 发起网络请求
		const response = await fetch(RawURL || atob('aHR0cHM6Ly9naXN0LmdpdGh1YnVzZXJjb250ZW50LmNvbS90aHVuZGVyMDc3NS9iZjFlODEwMWI2YTU1NGUyZGU2MzBjODYwOGYxMmFlOS9yYXcvYmlhbmxpYW5n'));
		// 获取响应的文本内容，这是一个异步操作
		const RawText = await response.text();
		const Textkey = await 变量数组整理(RawText);
		
		if (Textkey.TOKEN || env.TOKEN) 快速订阅访问入口 = await 整理(Textkey.TOKEN || env.TOKEN);
		...
		if (Textkey.ADDAPI || env.ADDAPI) addressesapi = await 整理(Textkey.ADDAPI || env.ADDAPI);
		...
		if (Textkey.PROXYIPAPI || env.PROXYIPAPI) {
			const proxyIPsapi = await 整理(Textkey.PROXYIPAPI || env.PROXYIPAPI);
		...
		if (快速订阅访问入口.length > 0 && 快速订阅访问入口.some(token => url.pathname === `/${token}`)) {
			...
			if (env.PASSWORD) {
				...
				if (env.KEY) {
					...
					uuid = Textkey.UUID || env.UUID || "null";
				}
			}

			path = Textkey.PATH || "/?ed=2560";
			sni = env.SNI || host;
			type = env.TYPE || type;
